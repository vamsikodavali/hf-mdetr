...
class MDETR(nn.Module):
    ...
    def __init__(
        ...
        # IMPLEMENT LOSS FUNCTIONS TO BE USED IN FORWARD FUNCTION
        matcher = build_matcher(args)
        self.weight_dict = {"loss_ce": args.ce_loss_coef, "loss_bbox": args.bbox_loss_coef}
        if args.contrastive_loss:
            self.weight_dict["contrastive_loss"] = args.contrastive_loss_coef
        if args.contrastive_align_loss:
            self.weight_dict["loss_contrastive_align"] = args.contrastive_align_loss_coef
        if args.predict_final:
            self.weight_dict["loss_isfinal"] = 1

        self.weight_dict["loss_giou"] = args.giou_loss_coef
        if args.masks:
            self.weight_dict["loss_mask"] = args.mask_loss_coef
            self.weight_dict["loss_dice"] = args.dice_loss_coef
        ...
        if args.contrastive_loss:
            self.contrastive_criterion = ContrastiveCriterion(temperature=args.temperature_NCE)
            self.contrastive_criterion.to(args.device)
        else:
            self.contrastive_criterion = None


    def forward(self, samples: NestedTensor, captions, targets, positive_map):
        ...
        if self.contrastive_criterion is not None:
            assert memory_cache is not None
            contrastive_loss = self.contrastive_criterion(memory_cache["text_pooled_op"], memory_cache["img_pooled_op"])
            loss_dict["contrastive_loss"] = contrastive_loss

        losses = sum(loss_dict[k] * self.weight_dict[k] for k in loss_dict.keys() if k in self.weight_dict)
        
        # TODO:  Implement HF_Model output class for return loss
